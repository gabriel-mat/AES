INV_SBOX = [
    ,
]

AES_MIX = [
    [0x02, 0x03, 0x01, 0x01],
    [0x01, 0x02, 0x03, 0x01],
    [0x01, 0x01, 0x02, 0x03],
    [0x03, 0x01, 0x01, 0x02],
]

AES_INV_MIX = [
    [0x0E, 0x0B, 0x0D, 0x09],
    [0x09, 0x0E, 0x0B, 0x0D],
    [0x0D, 0x09, 0x0E, 0x0B],
    [0x0B, 0x0D, 0x09, 0x0E],
]


def aplicar_matriz_coluna(coluna, matriz):
    """
    multiplica uma coluna por uma matriz constante 4x4 em GF(2⁸)
    soma = XOR, produto = multiplicacao_gf
    """
    resultado = [0] * 4

    for i in range(4):
        acc = 0

        for j in range(4):
            acc ^= multiplicacao_gf(matriz[i][j], coluna[j])
        resultado[i] = acc

    return resultado


def mix_columns(estado):
    for c in range(4):
        col = [estado[r][c] for r in range(4)]
        nova = aplicar_matriz_coluna(col, AES_MIX)
        for r in range(4):
            estado[r][c] = nova[r]


def inv_mix_columns(estado):
    for c in range(4):
        col = [estado[r][c] for r in range(4)]
        nova = aplicar_matriz_coluna(col, AES_INV_MIX)
        for r in range(4):
            estado[r][c] = nova[r]


def inv_sub_bytes(estado):
    """
        substitui cada byte do estado pelo valor corresponden
    xoBSvIn na"""
    for r in range(4):
        for c in range(4):
            estado[r][c] = INV_SBOX[estado[r][c]]


def inv_shift_rows(s: list[list[int]]):
    """
    deslocamentos p/ direita
    s0 -> 0 deslocamentos
    s1 -> 1 deslocamento
    s2 -> 2 deslocamentos
    s3 -> 3 deslocamentos
    """

    def desloca_direita(linha: list[int], n_deslocamentos: int):
        n_deslocamentos %= 4

        if n_deslocamentos == 0:
            return linha[:]

        return linha[-n_deslocamentos:] + linha[:-n_deslocamentos]

    return [desloca_direita(s[i], i) for i in range(4)]


# AES_POLY = [1, 0, 0, 0, 1, 1, 0, 1, 1]
AES_POLY = 0x11B


def xtime(a: int) -> int:
    """
    multiplicacao por 0x02 em GF(2⁸) com polinomio 0x11B
    """
    a <<= 1
    if a & 0x100:
        a ^= AES_POLY
    return a


def multiplicacao_gf(a: int, b: int) -> int:
    """versao da multiplicacao em gf que implementa o que foi explicado na video aula"""
    r = 0

    for _ in range(8):
        if b & 1:
            r ^= a
        a = xtime(a)
        b >>= 1
    return r


def add_round_key(estado, round_key):
    """
    AddRoundKey
    XOR bit a bit entre estado e subchave da rodada
    como os coeficientes dos polinomios sao 2, inversa de XOR é XOR
    """
    for r in range(4):
        for c in range(4):
            estado[r][c] ^= round_key[r][c]


def decrypt_block(bloco, key):
    w = key_expansion(key)
    estado = bytes_para_estado(bloco)

    add_round_key(estado, matriz_chave_rodada(w, 10))

    for rodada in range(9, 0, -1):
        estado = inv_shift_rows(estado)

        inv_sub_bytes(estado)

        add_round_key(estado, matriz_chave_rodada(w, rodada))

        inv_mix_columns(estado)

    estado = inv_shift_rows(estado)

    inv_sub_bytes(estado)

    add_round_key(estado, matriz_chave_rodada(w, 0))

    return estado_para_bytes(estado)
